...
export const useCachedStore = defineStore('cached', () => {
  ...
  const getBatchUserInfo = async (ids: number[]) => {
    const needRefresh = ids.filter((id) => !cacheTimestamps[id] || isDiffNow10Min(cacheTimestamps[id]))
    if (!needRefresh.length) return
    const results = await Promise.all(needRefresh.map((id) => apis.getUserById(id).send().catch(() => null)))
    results.forEach((user, i) => { if (user) { userCachedList[needRefresh[i]] = { ...user }; cacheTimestamps[needRefresh[i]] = Date.now() } })
  }

  const filterUsers = (searchKey: string) => atUsersList.value.filter((item) => item.nickname?.startsWith(searchKey))
  const filterUsersByIdList = (idList: number[]) => atUsersList.value.filter((user) => idList.includes(user.id))
