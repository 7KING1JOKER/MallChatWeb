...
const filtered = computed(() => {
  ...
  return serverStore.members.filter((m) => m.nickname.toLowerCase().includes(s))
})
</script>
<template>
  <div class="mention-picker">
    <div
      ...
      :key="m.userId"
      class="mention-item"
      @click="emit('select', m.nickname)"
      ><el-avatar :size="24" :src="m.avatar" /><span>{{ m.nickname }}</span></div
    >
