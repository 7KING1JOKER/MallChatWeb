...
const checkTimeInterval = (cur: any, pre: any) => {
  ...
  if ((pre && curTime - preTime > intervalTime) || computedCount >= computedCountMax) {
    computedCount = 0
    return { ...cur, timeBlock: timeToStr(curTime) }
  } else {
    computedCount += 1
    return cur
  }
...
export const isDiffNow = ({
  ...
  time: ConfigType
  diff: number
}) => dayjs().diff(dayjs(time), unit) > diff
export const isDiffNow10Min = (time: ConfigType): boolean =>
  isDiffNow({ time, unit: 'minute', diff: 10 })
